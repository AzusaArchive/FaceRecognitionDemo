...
# 加载人脸检测和姿势估计模型（dlib）
face_landmark_path = '../Assets/shape_predictor_68_face_landmarks.dat'
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            if len(face_rects) > 0:
                ...
                # locations = FaceRecognition.FindFaceLocations(frame)
                # landmarks = FaceRecognition.GetLandmarks(frame,locations)
                # 获取头部姿态
                reprojectdst, euler_angle = get_head_pose(shape)
                # if any(landmarks):
                #     reprojectdst, euler_angle = FaceRecognition.GetHeadPose(landmarks[0]) #face_recognition
                ...
                # FaceRecognition.DrawLandmarks(frame,landmarks) #face_recognition

                # 绘制正方体12轴
                for start, end in line_pairs:
                    p1 = reprojectdst[start]
                    p2 = reprojectdst[end]

...
import Final.Core.FaceRecognitionDlib as FaceRecDlib
from FaceInputPage import FaceInputPage
...
class AppDlib:
    def __init__(self, loop: AbstractEventLoop) -> None:
        ...
        self.btn_openVideo = tk.Button(self.header, text="打开视频文件", fg="#DDDDDD", bg="#323233", width=16,
                                       ...
                                       command=lambda: (self.SelectVideo, lambda: self.SwitchDetectionMode("Video")))
        ...
        self.drawFaceDataVar: tk.BooleanVar = tk.BooleanVar(value=True)
        self.drawHeadPosesVar: tk.BooleanVar = tk.BooleanVar(value=True)
        self.checkBox_drawLandmark = tk.Checkbutton(self.footer, text="绘制面部特征点", bg="#323233", fg="#DDDDDD",
                                                    selectcolor="#323233", activebackground="#646466",
                                                    activeforeground="#DDDDDD", variable=self.drawLandmarkVar)
        self.checkBox_drawLandmark.place(x=-20, y=0, relx=1, anchor="ne")
        self.checkBox_drawFaceData = tk.Checkbutton(self.footer, text="绘制人脸矩形框", bg="#323233",
                                                    fg="#DDDDDD",
                                                    selectcolor="#323233", activebackground="#646466",
                                                    activeforeground="#DDDDDD", variable=self.drawFaceDataVar)
        self.checkBox_drawFaceData.place(x=-140, y=0, relx=1, anchor="ne")
        self.checkBox_drawHeadPoses = tk.Checkbutton(self.footer, text="绘制头部姿态轮廓", bg="#323233",
                                                     fg="#DDDDDD",
                                                     selectcolor="#323233", activebackground="#646466",
                                                     activeforeground="#DDDDDD", variable=self.drawHeadPosesVar)
        self.checkBox_drawHeadPoses.place(x=-260, y=0, relx=1, anchor="ne")
        ...
        # FIXME: 将所有基于face_recognition的方法更换为Dlib
        self.detectionData: Optional[dict[str, DetectionInfo]] = dict()
        self.dlibHeadPoses: Optional[list[tuple[Any, Any]]] = None  # *** 使用Dlib获取的头部姿态
        self.dlibLandmarks = None  # *** 使用Dlib获取的特征点
    ...
    def InitControlPanel(self):
        ...
        slider_faceRecTolerance.grid(row=0, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="此值越大，人脸识别的匹配就越严格。（默认：0.45）", bg="#1E1E1E", fg="#DDDDDD") \
            .grid(row=1, column=0, padx=12, pady=(0, 24), sticky="nw")
        ...
        slider_eyeClosedThreshold.grid(row=3, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="此值越大，对眨眼的判定就越宽松。（默认：0.2）", bg="#1E1E1E", fg="#DDDDDD") \
            .grid(row=4, column=0, padx=12, pady=(0, 24), sticky="nw")
        ...
        slider_mouseOpenedThreshold.grid(row=5, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="此值越大，对张嘴的判定就越严格。（默认：0.5）", bg="#1E1E1E",
                 fg="#DDDDDD") \
            .grid(row=6, column=0, padx=12, pady=(0, 24), sticky="nw")
        ...
        slider_expressionCheckFrequency.grid(row=7, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="此值越大，面部状态的检测就越精准（可能影响性能）。\n（默认：30）", bg="#1E1E1E",
                 fg="#DDDDDD") \
            .grid(row=8, column=0, padx=12, pady=(0, 24), sticky="nw")
        ...
        slider_asleepTimeThreshold.grid(row=9, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="对象闭眼时间超过该设定值后判定为睡觉状态。（默认：60）", bg="#1E1E1E",
                 fg="#DDDDDD") \
            .grid(row=10, column=0, padx=12, pady=(0, 24), sticky="nw")
        ...
        slider_headDownAngleThreshold.grid(row=11, column=0, padx=12, pady=5, sticky="nw")
        tk.Label(controlPanel, text="对象头部俯仰角超出该值时判定为低头状态（默认：10）", bg="#1E1E1E",
                 fg="#DDDDDD") \
            .grid(row=12, column=0, padx=12, pady=(0, 24), sticky="nw")
    ...
    async def MainUpdateLoopAsync(self, mode: Literal["Camera", "Video"]):
        ...
        async def processPipeline(waitTime: float):
            self.currentFrame = self.frameCapture.GetFrame()
            self.RecognizeFaces()
            self.GetLandmarks()
            self.GetHeadPose()
        ...
        try:
            ...
            if mode.upper() == "CAMERA":
                with await Frame.FromCamera() as self.frameCapture:
                    ...
                    while True:
                        await processPipeline(waitTime)
    ...
    def GetLandmarks(self):
        ...
        self.dlibLandmarks = FaceRecDlib.GetLandMarks(self.currentFrame, locations)

    def GetHeadPose(self):
        self.dlibHeadPoses = []

        for landmark in self.dlibLandmarks:
            self.dlibHeadPoses.append(FaceRecDlib.GetHeadPose(landmark))

    def DetectExpression(self):  # 可转为异步降低检测频率以减少性能消耗
        if self.dlibLandmarks is not None and any(self.dlibLandmarks):
            for i in range(len(self.dlibLandmarks)):
                if self.faceData[i][0] != "Unknown":
                    info = self.detectionData[self.faceData[i][0]]
                    if FaceRecDlib.IsEyesClosed(self.dlibLandmarks[i], self.eyeClosedThresholdVar.get()):
                        info.EyeClosed = True
                    else:
                        info.EyeClosed = False

                    if FaceRecDlib.IsMouseOpened(self.dlibLandmarks[i], self.mouseOpenedThresholdVar.get()):
                        info.MouseOpened = True
                    else:
                        info.MouseOpened = False

                    if FaceRecDlib.IsHeadDown(self.dlibHeadPoses[i][1], self.headDownAngleThresholdVar.get()):
                        info.HeadDown = True
                    else:
                        info.HeadDown = False

    def DrawDetectionData(self):
        if self.drawFaceDataVar.get() and self.faceData is not None:
            for name, location in self.faceData:
    ...
    def DrawLandmarks(self):
        if self.drawLandmarkVar.get():
            FaceRecDlib.DrawLandmarks(self.currentFrame, self.dlibLandmarks)

    def DrawHeadPoses(self):
        if self.drawHeadPosesVar.get() and self.dlibHeadPoses is not None:
            for headpose in self.dlibHeadPoses:
...
async def Main():
    await AppDlib(asyncio.get_event_loop()).RunAsync()
